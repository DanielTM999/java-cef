...
bool Context::Initialize(JNIEnv* env,
...
#if defined(OS_WIN)
  // Orion fork: use a multi-threaded message loop on Windows even when OSR is
  // enabled. Under an external message pump a windowed browser (e.g. the native
  // DevTools popup) has no thread pumping its OS messages, so Windows marks it
  // "Not Responding". OSR is fully supported under a multi-threaded message
  // loop, so this keeps offscreen rendering working while making windowed
  // popups responsive.
  ...
  if (!external_message_pump_) {
    // Windowed rendering on Linux requires multi-threaded message loop,
    // otherwise something eats the messages required by Java and the Java
    // window becomes unresponsive. Note that however, on Linux it is more
    // difficult to get this to work: it is necessary for the first call to
    // Xlib to be a call to XInitThreads! Since Java itself calls Xlib when it
    // initializes the first window, an application must make sure to invoke
    // this method before any other Xlib functions are called - including by
    // the Java runtime itself, which makes this feat a little tricky. The
    // CefApp class exposes a static method for this purpose,
    // initXlibForMultithreading(), but the host application must load the jcef
    // native lib by itself in order to use it, and it must invoke it VERY
    // early, ideally at the beginning of the main method. Another neat trick
    // to get this done is to create a special native lib just for this purpose
    // like described in this StackOverflow thread:
